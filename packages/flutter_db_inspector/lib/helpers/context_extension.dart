...
extension ContextExtension on BuildContext {
  ...
  /// A grid delegate with a fixed cross-axis count that adapts based on device size categories
  SliverGridDelegateWithFixedCrossAxisCount get gridDelegate {
    final shortest = MediaQuery.sizeOf(this).shortestSide;
    final isPortraitMode = isPortrait;

    int crossAxisCount;
    if (shortest < 400) {
      // Mobile Small
      crossAxisCount = isPortraitMode ? 1 : 2;
    } else if (shortest < 600) {
      // Mobile
      crossAxisCount = isPortraitMode ? 2 : 4;
    } else if (shortest < 1000) {
      // Tablet
      crossAxisCount = isPortraitMode ? 3 : 5;
    } else {
      // Desktop
      crossAxisCount = isPortraitMode ? 4 : 6;
    }
